...
| Semantic mem | SQLite cosine index        | Qdrant / pgvector           |
...
app/
  ...
  tools/      реестр инструментов: безопасный калькулятор + веб-поиск (DuckDuckGo)
...
`/menu` (и `/start`) открывает inline-меню: **Модель / Стиль / Статус / Сброс**. Выбор
модели и персоны — тоже кнопками (по нажатию бот меняет настройку и редактирует сообщение).
Ответ агента несёт список кнопок (`AgentResponse.buttons`) в channel-agnostic виде; Telegram
рисует их клавиатурой, а нажатие повторно входит в конвейер как команда `/{action}` — то есть
кнопки переиспользуют те же обработчики, что и текстовые команды.

### Прочие команды

- `/status` — показать текущую модель и стиль.
- `/reset` — очистить историю разговора (сессия + семантика + БД-диалог); модель и стиль сохраняются.
- `/help` — справка со списком команд.

В Telegram команды регистрируются в меню (`setMyCommands`) — всплывают при вводе `/`.
...
- **Rate-limiting** (`app/core/ratelimit.py`): на пользователя, два скользящих окна —
  `RATE_LIMIT_PER_MINUTE` (сглаживает всплески) и `RATE_LIMIT_PER_DAY` (ограничивает
  дневной расход). Команды (`/status`, `/model`…) не считаются — лимит только на
  запросы к модели. In-process; в мульти-инстансе заменяется на Redis.
...
- **Постоянная семантическая память** (`app/memory/semantic.py`): фрагменты и их
  эмбеддинги хранятся в таблице `semantic_records`, поэтому ассоциативная память
  переживает перезапуск (раньше жила только в RAM). `/reset` её очищает.
- **Веб-поиск** (`app/tools/websearch.py`): бесплатный, без ключа (DuckDuckGo).
  Планировщик по эвристике намерения («где взять…», «ссылка», «новости», URL…)
  добавляет шаг поиска; результаты (заголовки + URL + сниппеты) уходят в промпт,
  и модель отвечает со ссылками. Блокирующий вызов идёт в отдельном потоке.
